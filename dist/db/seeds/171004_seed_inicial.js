...
async function tableExists(q, table) {
    const r = await q.query(`SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name=$1 LIMIT 1`, [table]);
    return r.length > 0;
}
async function upsertByNombre(q, table, values) {
    if (!(await tableExists(q, table)))
        return;
    for (const v of values) {
        await q.query(`INSERT INTO ${table} (nombre) VALUES ($1)
       ON CONFLICT (nombre) DO NOTHING;`, [v]);
    }
}
async function main() {
    ...
    `);
        ...
        if (hasDepartamentos && hasMunicipios) {
            ...
            await q.query(`INSERT INTO departamentos (nombre)
     SELECT $1
     WHERE NOT EXISTS (SELECT 1 FROM departamentos WHERE nombre = $1);`, ['Huehuetenango']);
            // Obtén su UUID
            const depRes = await q.query(`SELECT departamento_uuid FROM departamentos WHERE nombre = $1 LIMIT 1`, ['Huehuetenango']);
            ...
            if (deptoUuid) {
                ...
                for (const m of municipiosHuehue) {
                    await q.query(`INSERT INTO municipios (departamento_uuid, nombre)
         SELECT $1, $2
         WHERE NOT EXISTS (
